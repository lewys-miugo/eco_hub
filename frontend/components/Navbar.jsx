  ...
  return (
    <nav className="fixed w-full z-50 bg-[#163473] shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {/* Logout Button */}
            <button className="px-4 py-2 bg-[#D4AF37] text-[#163473] text-sm font-medium rounded-md hover:bg-[#B8941F] transition-colors">
              Logout
            </button>
          </div>
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1 bg-[#163473] shadow-lg">
          {navLinks.map((link) => (
            <Link
              key={link.key}
              href={link.path}
